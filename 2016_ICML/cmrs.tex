...
\usepackage{umoline}
%\newcommand{\todo}[2]{\Underline{#1}\textbf{ [\scriptsize TODO: #2] }}
\newcommand{\todo}[2]{#1}
...
expected immediate regret of its ultimate recommendation for the optimum. While empirically ES and MRS perform similar in most of the
cases, MRS produces fewer outliers with high regret than ES. We provide empirical
...
model-based, global approach for optimizing black-box functions that are
non-convex, do not necessarily provide derivatives, are expensive to evaluate (either
computationally, economically, or morally), and are potentially noisy. Under
...
at finding a solution which has small regret with regard to the true optimum. BO
aims at finding such a solution of minimum regret while  minimizing at the same
...
BO has been applied to a diverse set of problems, ranging from automatic  machine
learning \cite{bergstra_algorithms_2011,snoek_practical_2012} over robotics
...
\cite{villemonteix_informational_2008, hennig_entropy_2012,hernandez-lobato_predictive_2014}, which aims at maximizing the information gain regarding
...
extensive review of BO. BO can be applied to black-back optimization
...
a global method which makes use of all historic evaluations of $f(\mathbf{x})$
...
\citep[PES,][]{hernandez-lobato_predictive_2014}. PES can be approximated more
efficiently, does not require discretization, and has obtained empirical results
that are as least as good as discretized ES with expectation propagation. A further advantage of PES is that it allows a formal treatment of GP hyperparameters.
...
Let $X \subset \mathcal{R}^D$ be some bounded domain and $f: \mathcal{X} \mapsto
